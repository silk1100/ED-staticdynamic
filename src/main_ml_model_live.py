...
from preprocess.target_cleaner import clean_target, get_all_target_flags, get_target_flags, set_admitted_discharged_only
...
PATH = "/work/InternalMedicine/s223850/ED-StaticDynamic/static_dynamic_feats_ID1/tr_8_te_4/"
def get_index_id(files, idx):
    idx_dict = defaultdict(list)
    for f in files:
        if int(f.split('_')[1]) not in idx:
            continue    
        idx_dict[int(f.split('_')[1])].append(f)
    for key in idx_dict:
        v = sorted(idx_dict[key], key=lambda x:int(x.split('_')[3].split('.')[0]), reverse=True)
        idx_dict[key] = v
    for key, files in idx_dict.items():
        for f in files:
            if 'static' in f:
...
def feature_selector(df_train_all):
    pass

def optimizer(df_train_all):
    pass

def train(clc, df_train_all, df_test_all):
    pass

def run_live(clc, df_test_all):
    pass

path_clean = '/work/InternalMedicine/s223850/ED-StaticDynamic/clean_target'

if __name__ == "__main__":
    do_feature_selection = True
    do_hyperparams_optimization = True
    df_raw = pd.read_csv(constants.RAW_DATA)
    df_raw_admit = df_raw[df_raw['ED_Disposition']=='Admitted']
    df_raw_disch = df_raw[df_raw['ED_Disposition']=='Discharged']
    df_raw_1 = pd.concat([df_raw_admit, df_raw_disch], axis=0, ignore_index=True)
    admi_flags = get_target_flags(df_raw_1, 'EVENT_NAME', 'ED_Disposition', 'admi', True, 0.7)
    disch_flags = get_target_flags(df_raw_1, 'EVENT_NAME', 'ED_Disposition', 'disch', True, 0.7)
    bed_flags = get_target_flags(df_raw_1, 'EVENT_NAME', 'ED_Disposition', 'bed', True, 0.7)
    consul_flags = get_target_flags(df_raw_1, 'EVENT_NAME', 'ED_Disposition', 'consul', True, 0.7)
    obser_flags = get_target_flags(df_raw_1, 'EVENT_NAME', 'ED_Disposition', 'obser', True, 0.7)
    all_flags = np.concatenate([admi_flags, bed_flags, disch_flags, consul_flags, obser_flags])
    all_flags_list = list(set(all_flags))
    df_pat_list, errors_list, order_before_flag = clean_target(df_raw_1, all_flags, normalize_event_col=True,
                                                               min_date=pd.to_datetime("11-01-2022", dayfirst=False))
    df_clean = pd.concat(df_pat_list, axis=0, ignore_index=True)
    df_clean.to_csv(os.path.join(constants.OUTPUT_DIR, 'clean_target', 'df_clean_12-21-23.csv')) 
    x = 0
    # Determine the beginning of the training
    # Start at June 2022 
    train_lists, tests_lists = get_index_id([f for f in os.listdir(PATH) if f.endswith('.csv')], np.arange(240, 251))
    df_train_all = []
    df_test_all = []

    for tr in train_lists:
        df_tr = pd.read_csv(os.path.join(PATH, tr))
        df_train_all.append(df_tr)

    for te in tests_lists:
        df_te = pd.read_csv(os.path.join(PATH, te))
        df_test_all.append(df_te)

    df_train_all = pd.concat(df_train_all, axis=0)
    df_test_all = pd.concat(df_test_all, axis=0)

    # End at September 2023

    # Determine the end of the training and the beginning of the testing set

    # Determine the modules used (feature selection, hyper parameter optimization)
    if do_feature_selection:
        selector = feature_selector(df_train_all)
        df_train_all = selector.transform(df_train_all)
        df_test_all = selector.transform(df_test_all)
    
    if do_hyperparams_optimization:
        clc = optimizer(df_train_all)
        clc, performance = train(clc, df_train_all, df_test_all) 
    else:
        clc, performance = train('all', df_train_all, df_test_all) 
    
    run_live(clc, df_test_all)
